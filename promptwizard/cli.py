...
Layout follows the product requirement: one default command that does the whole
job (``promptwizard "<prompt>"``), plus small commands for the things a user
needs around it (``providers``, ``sessions``, ``config``, ``gui``).

Every user-facing string comes from the locale catalog, and every error is
rendered as ``message`` + localized ``hint`` — the top-level handler is the only
place that prints, so no raw traceback ever reaches the terminal.
"""
...
class Printer:
    """Prints progress and result lines, honouring ``--quiet``."""
...
#: Flags that consume the following token, used to tell a command name from a value.
...
def _prescan_lang(argv: Sequence[str]) -> str | None:
    """Find ``--lang`` before argparse runs, so help text is already localized."""
...
def _split_command(argv: Sequence[str]) -> tuple[str, list[str]]:
    """Separate the optional command name from the rest of the arguments.

    Means both ``promptwizard providers`` and ``promptwizard --quiet providers``
    work: the scan skips flags and their values so ``--provider groq providers``
    does not mistake ``groq`` for the command.  The first real positional decides;
    anything that is not a known command is treated as the prompt of ``run``.
    """
    index = 0
...
def build_parser(translator: Translator, command: str = "run") -> argparse.ArgumentParser:
    """Build the parser for one command, with localized help."""
    ...
    # Every command accepts the same flags: it keeps `promptwizard --provider x providers`
    # parseable, and flags a command ignores are harmless.
    parents = [common, llm]
    parser = argparse.ArgumentParser(
        prog=prog,
        description=translator("cli.description"),
        epilog=translator("cli.epilog"),
...
def build_config(args: argparse.Namespace, command: str) -> Config:
    """Merge defaults, config file, env and CLI flags into the effective config."""
...
def _read_prompt(args: argparse.Namespace, translator: Translator) -> str:
    if getattr(args, "file", None):
        ...
        try:
            ...
            raise InputError(
                f"cannot read the prompt file {path}: {exc}",
                hint_key="error.input.file",
                hint_kwargs={"path": str(path)},
            ) from exc
...
def _ask_batch(questions: Sequence[Question], translator: Translator) -> dict[str, str]:
    """Render the whole batch, then read one line per question."""
    print()
        ...
        for index, issue in enumerate(result.analysis.issues, start=1):
            printer.out(
                translator(
                    "run.issue_item",
                    index=index,
                    severity=translator(f"severity.{issue.severity}"),
                    category=translator(f"category.{issue.category}"),
                    title=issue.title,
                )
            )
            if issue.detail:
...
def _cmd_run(args: argparse.Namespace, config: Config, translator: Translator, printer: Printer) -> int:
    ...
    if interactive and not args.no_questions and args.max_questions != 0:
        ask = lambda questions: _ask_batch(questions, translator)  # noqa: E731 - trivial adapter
    session = run(
        config,
        translator,
        prompt,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        return 130
    except Exception as exc:  # noqa: BLE001 - last resort: no traceback to the user
