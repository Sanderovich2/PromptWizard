"""Rendering a :class:`~promptwizard.pipeline.SessionResult` to md / txt / json."""
...
def to_markdown(result: SessionResult, translator: Translator) -> str:
    """A readable Markdown report: prompt, issues, questions, rewrite, changes."""
...
def to_text(result: SessionResult, translator: Translator) -> str:
    """Same information as Markdown, with plain ``====`` separators."""
    rewrite = _rewrite(result)
    bar = "=" * 60
...
def to_json(result: SessionResult) -> str:
    """The session record as pretty JSON (the same shape stored on disk)."""
    return json.dumps(result.to_dict(), ensure_ascii=False, indent=2) + "\n"


def render(result: SessionResult, fmt: str, translator: Translator) -> str:
    """Render ``result`` in ``fmt`` (``md``/``txt``/``json``; ``markdown``/``text`` aliases)."""
...
def write_output(text: str, path: str | Path) -> Path:
    """Write ``text`` to ``path`` (UTF-8, parents created) and return the path."""
...
def session_to_markdown(record: dict[str, Any], translator: Translator) -> str:
    """Render a stored record (``sessions --show``) reusing the export format."""
