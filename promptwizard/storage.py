"""Session history: append-only JSONL under ``<home>/sessions/sessions.jsonl``.

JSONL is deliberate: a crashed run cannot corrupt earlier records, the file is
greppable, and ``--out`` exports stay independent of it.  Records are plain
dicts (:meth:`promptwizard.pipeline.SessionResult.to_dict`), so this module has
...
def save_session(config: Config, record: Mapping[str, Any]) -> Path:
    """Append one session record and return the file it went to."""
    ...
    try:
        ...
        raise StorageError(
            f"cannot write the session log at {path}: {exc}",
            hint_key="error.storage.write",
        ) from exc
    return path


def iter_sessions(config: Config) -> Iterator[dict[str, Any]]:
    """Yield every stored session, oldest first, skipping unreadable lines."""
    path = sessions_file(config)
    if not path.exists():
        return
    try:
        ...
        raise StorageError(
            f"cannot read the session log at {path}: {exc}",
            hint_key="error.storage.read",
        ) from exc
...
def list_sessions(config: Config, limit: int | None = None) -> list[dict[str, Any]]:
    """Stored sessions, newest first, optionally capped at ``limit``."""
...
def get_session(config: Config, session_id: str) -> dict[str, Any] | None:
    """Return one session by id, or ``None``. Accepts a unique id prefix."""
