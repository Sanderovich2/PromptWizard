...
One implementation covers every provider that speaks the
``/chat/completions`` dialect: Groq and OpenRouter (thin subclasses in this
package), plus *any* new free tier the user adds through config alone::
...
No code change is needed for that, which is the requirement the abstraction
exists to satisfy.
"""
...
class OpenAICompatibleProvider(LLMProvider):
    """Chat completions against any OpenAI-compatible ``base_url``."""

    name = "openai_compatible"
    kind = "openai_compatible"
    requires_key = True
    #: Providers that reject ``response_format`` should set this to False.
    supports_json_mode = True
    #: Extra headers (OpenRouter asks for attribution headers, for example).
    ...
    def _require_ready(self) -> None:
        if not self.base_url:
            raise MissingCredentials(
                f"{self.name}: no base_url configured",
                hint_key="error.provider.no_base_url",
        ...
        if self.requires_key and not self.api_key:
            raise MissingCredentials(
                ...
                hint_kwargs={"provider": self.name, "env_var": self.api_key_env or "API_KEY"},
            )
        if not self.model:
            raise MissingCredentials(
                f"{self.name}: no model configured",
    ...
    def list_models(self) -> tuple[str, ...]:
        """Return the model ids the account can use (``GET /models``)."""
        data = get_json(
    ...
    def status(self) -> ProviderStatus:
        ...
        except Exception as exc:  # noqa: BLE001 - a probe failure must not raise
            status = getattr(exc, "status", None)
            if status == 404:
                # Endpoint reachable but provider does not expose /models.
                return ProviderStatus(
                    name=self.name,
                    available=True,
                    detail="key present; provider does not expose /models",
                    requires_key=True,
                    key_present=True,
                )
            return ProviderStatus(
    ...
    def _extract_text(data: Mapping[str, Any]) -> str:
        ...
        if error and not data.get("choices"):
            ...
            raise LLMResponseError(
                f"provider returned an error: {detail}",
        ...
        if content is None and isinstance(first.get("text"), str):
            content = first["text"]  # legacy completion shape
