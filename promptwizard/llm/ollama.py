"""Ollama adapter - local models, no API key, no network, no cost.

Used when an Ollama server answers on ``http://localhost:11434`` (override with
``OLLAMA_HOST`` or the ``providers.ollama.base_url`` config field).
"""
...
class OllamaProvider(LLMProvider):
    """Chat with a model served by a local Ollama instance."""
    ...
    def complete(self, request: LLMRequest) -> LLMResponse:
        ...
        payload: dict[str, Any] = {
            ...
            "options": {
                ...
                "num_predict": prepared.max_tokens,
            },
        }
        ...
        try:
            ...
            if exc.status == 404 and "not found" in exc.body.lower():
                raise ProviderHTTPError(
                    f"ollama: model {self.model!r} is not pulled locally",
                    status=exc.status,
                    body=exc.body,
                    hint_key="error.provider.ollama_pull",
                    hint_kwargs={"provider": self.name, "model": self.model},
                ) from exc
            raise
        return LLMResponse(
    ...
    def list_models(self) -> tuple[str, ...]:
        """Return the model tags available locally."""
    ...
    def status(self) -> ProviderStatus:
        ...
        except Exception as exc:  # noqa: BLE001 - status must degrade, never raise
    ...
    def _extract_text(data: Mapping[str, Any]) -> str:
        ...
        if isinstance(message, Mapping):
            ...
            if isinstance(content, str) and content.strip():
                return content
        # /api/generate style payloads (some proxies answer with this shape)
