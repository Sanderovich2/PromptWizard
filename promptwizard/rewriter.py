...
__all__ = ['Rewrite', 'build_system_prompt', 'build_user_prompt', 'heuristic', 'parse']
_QUESTION_TO_SECTION: dict[str, str] = {'context': 'context', 'audience': 'audience', 'constraints': 'constraints', 'format': 'format', 'length': 'format', 'examples': 'examples', 'tone': 'tone'}
...
def parse(text: str, language: str='unknown') -> Rewrite:
    ...
    if not isinstance(data, Mapping):
        raise LLMResponseError('rewriter: the model did not return a JSON object', hint_key='error.provider.bad_response', hint_kwargs={'provider': 'api'})
    improved = as_str(data.get('improved_prompt')) or as_str(data.get('prompt'))
    if not improved:
        raise LLMResponseError('rewriter: the model returned no improved prompt', hint_key='error.provider.bad_response', hint_kwargs={'provider': 'api'})
    changes: list[str] = []
...
def heuristic(prompt: str, analysis: Analysis, answers: Mapping[str, str] | None, prompt_translator: Translator, ui_translator: Translator) -> Rewrite:
    ...
    return Rewrite(improved_prompt=improved_prompt, changes=tuple(changes), language=analysis.language, raw='')
