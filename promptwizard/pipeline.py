...
class Session:
    ...
    def analyze(self) -> Analysis:
        ...
        if not self.offline:
            text, reason = _try_complete(self.provider, self.translator, LLMRequest(prompt=analysis_user(self.prompt), system=analysis_system(self.translator), json_mode=True))
            if text:
                try:
    ...
    def make_questions(self, *, limit: int | None=None, no_questions: bool=False) -> tuple[Question, ...]:
        ...
        if self.offline:
            ...
            text, reason = _try_complete(self.provider, self.translator, LLMRequest(prompt=questions_user(self.prompt, analysis), system=questions_system(self.translator, effective_limit), json_mode=True))
            if text:
                try:
    ...
    def run_rewrite(self) -> Rewrite:
        ...
        rewrite: Rewrite | None = None
        if not self.offline:
            text, reason = _try_complete(self.provider, self.translator, LLMRequest(prompt=rewrite_user(self.prompt, analysis, self.questions, self.answers), system=rewrite_system(self.translator, self.prompt_language), json_mode=True))
            if text:
                try:
    ...
    def result(self) -> SessionResult:
        ...
        return SessionResult(id=new_session_id(), started_at=self.started_at, finished_at=_now(), provider=self.provider.name, model=self.provider.model or '-', mode=self.mode(), prompt_language=self.prompt_language, ui_language=self.config.lang, original_prompt=self.prompt, analysis=analysis, questions=self.questions, answers=dict(self.answers), rewrite=self.rewrite, warnings=tuple(self.warnings))
