...
def launch(config: Config, translator: Translator) -> int:
    try:
        ...
        raise GUIUnavailable('tkinter is not available in this Python build', hint_key='error.gui.unavailable') from exc
    try:
        root = tk.Tk()
...
class PromptWizardWindow:
    ...
    def _build(self) -> None:
        ...
        self.lang_combo = ttk.Combobox(top, textvariable=self.lang_var, values=list(LANGUAGES), width=6, state='readonly')
    ...
    def _show_analysis(self, session: Session, questions: tuple[Question, ...]) -> None:
        ...
        if analysis is not None:
            ...
            for index, issue in enumerate(analysis.issues, start=1):
                lines.append(self.translator('run.issue_item', index=index, severity=self.translator(f'severity.{issue.severity}'), category=self.translator(f'category.{issue.category}'), title=issue.title))
                if issue.detail:
        ...
        for index, question in enumerate(questions):
            ...
            self.ttk.Label(self.questions_frame, text=title, wraplength=680, justify='left').grid(row=index * 2, column=0, columnspan=2, sticky='w', pady=(4, 0))
    ...
    def _on_rewrite(self) -> None:
        if self.session is None:
            return
        answers = {question.id: entry.get() for question, entry in zip(self.session.questions, self.answer_entries)}
    ...
    def _on_save(self) -> None:
        ...
        if self.result is None or self.result.rewrite is None:
            return
        path = filedialog.asksaveasfilename(title=self.translator('gui.save_dialog'), defaultextension='.txt', filetypes=[('Text', '*.txt'), ('Markdown', '*.md'), ('All files', '*.*')])
        if not path:
            return
        try:
