"""Rewriter: JSON parsing, change formatting, and the deterministic scaffold."""
...
def test_parse_reads_changes_and_joins_what_and_why():
    text = (
        '{"improved_prompt":"Task: x","language":"en",'
        '"changes":[{"what":"Added format","why":"clearer"},{"what":"Trimmed"}]}'
    )
...
def _analysis() -> Analysis:
    return Analysis(
        ...
        score=40,
        summary="",
        issues=(Issue("format", "high", "t"), Issue("audience", "medium", "t")),
    )
...
def test_heuristic_builds_a_scaffold_and_folds_in_answers(translator):
    rewrite = heuristic(
        "Do the thing",
        _analysis(),
        {"format": "a table", "audience": "engineers"},
        translator,
        translator,
    )
