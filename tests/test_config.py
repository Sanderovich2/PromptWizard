...
def test_defaults_come_from_the_builtin_table(tmp_path):
    ...
    assert {'pollinations', 'ollama', 'gemini', 'groq', 'openrouter', 'openai_compatible', 'offline'} <= set(config.providers)
...
def test_config_file_overrides_defaults(tmp_path):
    (tmp_path / 'config.json').write_text(json.dumps({'provider': 'groq', 'max_questions': 3}), encoding='utf-8')
    config = Config.load(home=tmp_path, dotenv=False)
...
@pytest.mark.parametrize('overrides', [{'temperature': 5.0}, {'timeout': 0}, {'max_tokens': 0}, {'max_questions': 99}])
...
def test_unknown_provider_is_usable_through_config_alone(tmp_path):
    (tmp_path / 'config.json').write_text(json.dumps({'providers': {'cerebras': {'base_url': 'https://api.cerebras.ai/v1', 'model': 'llama3.1-8b', 'api_key_env': 'CEREBRAS_API_KEY'}}}), encoding='utf-8')
